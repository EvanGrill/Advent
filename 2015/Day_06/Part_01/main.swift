...
func turnOn( grid: inout [[Bool]], topLeft: Pair, bottomRight: Pair ) {
...
func turnOff( grid: inout [[Bool]], topLeft: Pair, bottomRight: Pair ) {
	for i in topLeft.x ... bottomRight.x {
		for j in topLeft.y ... bottomRight.y {
			grid[i][j] = false
		}
...
func toggle( grid: inout [[Bool]], topLeft: Pair, bottomRight: Pair ) {
	for i in topLeft.x ... bottomRight.x {
		for j in topLeft.y ... bottomRight.y {
			grid[i][j] = !grid[i][j]
		}
...
func parseInstruction( grid: inout [[Bool]], input: String ) {
	let split = input.split( separator: " " )
	if( split[0] == "turn" ) {
		var pair = String( split[2] )
		var X = Int( pair.split( separator: "," )[0] )
		var Y = Int( pair.split( separator: "," )[1] )
		let topLeft = Pair( x: X, y: Y )
		pair = String( split[4] )
		X = Int( pair.split( separator: "," )[0] )
		Y = Int( pair.split( separator: "," )[1] )
		let bottomRight = Pair( x: X, y: Y )
		if( split[1] == "on" ) {
			turnOn( grid: &grid, topLeft: topLeft, bottomRight: bottomRight )
		}
		else {
			turnOff( grid: &grid, topLeft: topLeft, bottomRight: bottomRight )
		}
	}
	else {
		var pair = String( split[1] )
		var X = Int( pair.split( separator: "," )[0] )
		var Y = Int( pair.split( separator: "," )[1] )
		let topLeft = Pair( x: X, y: Y )
		pair = String( split[3] )
